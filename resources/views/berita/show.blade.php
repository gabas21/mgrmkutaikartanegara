        ...
        <div class="max-w-4xl mx-auto px-6 lg:px-8">
            ...
            <div class="mt-16 pt-8 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-3"
                     x-data="{
                        pageUrl: '{{ url()->current() }}',
                        pageTitle: {{ Js::from($berita->title) }},
                        copied: false,
                        shareFacebook() {
                            window.open('https://www.facebook.com/sharer/sharer.php?u=' + encodeURIComponent(this.pageUrl), '_blank', 'width=600,height=400');
                        },
                        shareTwitter() {
                            window.open('https://twitter.com/intent/tweet?url=' + encodeURIComponent(this.pageUrl) + '&text=' + encodeURIComponent(this.pageTitle), '_blank', 'width=600,height=400');
                        },
                        copyLink() {
                    ...
                    <button @click="shareFacebook()"
                            ...
                            class="w-10 h-10 rounded-full bg-slate-50 hover:bg-blue-50 hover:text-blue-600 text-slate-400 flex items-center justify-center active:scale-95 touch-manipulation transition-all duration-300">
                    ...
                    <button @click="shareTwitter()"
                            ...
                            class="w-10 h-10 rounded-full bg-slate-50 hover:bg-sky-50 hover:text-sky-500 text-slate-400 flex items-center justify-center active:scale-95 touch-manipulation transition-all duration-300">
                    ...
                    <button @click="copyLink()"
                            ...
                            class="w-10 h-10 rounded-full bg-slate-50 hover:bg-green-50 hover:text-green-500 text-slate-400 flex items-center justify-center active:scale-95 touch-manipulation transition-all duration-300 relative"
                            :class="copied ? 'bg-green-50 text-green-500' : ''">
                        {{-- Icon: link --}}
                        ...
                        {{-- Icon: checkmark saat tersalin --}}
                        <svg x-show="copied" class="w-4 h-4 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                        {{-- Toast "Tersalin!" --}}
                        <span x-show="copied" x-transition
                              class="absolute -top-9 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-xs font-bold px-2.5 py-1 rounded-lg whitespace-nowrap pointer-events-none">
                            Tersalin!
                        </span>
                    </button>
                </div>
